...
#endif
/* Write status register and ensure bits in mask match written values */
static int write_lock_and_check(struct spi_nor *nor, loff_t to, u8 cmd)
{
	int ret;
#ifdef WLOCK_CHECK
	int check_len;
					...
					loff_t ofs, uint64_t len, u8 cmd)
{
	...
	case SNOR_MFR_XTX:
		while (ofs_lock < ofs_lock_end) {
			...
			ret = write_lock_and_check(nor, ofs_lock, cmd);
			if (ret)
...
static int sunxi_individual_lock(struct spi_nor *nor, loff_t ofs, uint64_t len)
{
	...
	return sunxi_individual_handle_lock(nor, ofs, len, SPINOR_OP_IBLK);
}
					...
					loff_t ofs, uint64_t len)
{
	...
	return sunxi_individual_handle_lock(nor, ofs, len, SPINOR_OP_UIBLK);
}
